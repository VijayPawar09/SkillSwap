...
const Chats = () => {
  ...
  const handleRequestReject = async () => {
    ...
    try {
      setAcceptRequestLoading(true);
      const { data } = axios.post("http://localhost:8000/request/rejectRequest", { requestId: selectedRequest._id });
      console.log(data);
      toast.success(data.message);
      setRequests((prevState) => prevState.filter((request) => request._id !== selectedRequest._id));
    } catch (err) {
            ...
            <Form>
              ...
              <Button
                ...
                onClick={async (e) => {
                  ...
                  try {
                    const { data } = await axios.post("http://localhost:8000/user/sendScheduleMeet", scheduleForm);
